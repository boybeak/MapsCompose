...
internal class MapPropertiesNode(
    ...
    override fun onAttached() {
        ...
        map.setOnIndoorStateChangeListener(object : IndoorStateChangeListener {
            override fun onIndoorBuildingFocused() {
                clickListeners.indoorStateChangeListener.onIndoorBuildingFocused()

            }

            override fun onIndoorLevelActivated(building: IIndoorBuilding) {
...
internal inline fun MapUpdater(
    ...
    ComposeNode<MapPropertiesNode, MapApplier>(
        ...
        update(contentDescription) { this.contentDescription = contentDescription }

        set(locationSource) { map.setLocationSource(it) }
        set(mapProperties.isBuildingEnabled) { map.isBuildingEnabled = it }
        ...
        // TODO
        set(mapUiSettings.compassEnabled) { map.uiSettings.compassEnabled = it }
        set(mapUiSettings.indoorLevelPickerEnabled) { map.uiSettings.indoorLevelPickerEnabled = it }
        set(mapUiSettings.mapToolbarEnabled) { map.uiSettings.mapToolbarEnabled = it }
        set(mapUiSettings.myLocationButtonEnabled) { map.uiSettings.myLocationButtonEnabled = it }
        set(mapUiSettings.rotationGesturesEnabled) { map.uiSettings.rotationGesturesEnabled = it }
        set(mapUiSettings.scrollGesturesEnabled) { map.uiSettings.scrollGesturesEnabled = it }
        ...
        update(cameraPositionState) { this.cameraPositionState = cameraPositionState }
